var pollyConnection = require('../../core/connection');
...
describe('Check if language settings are correct and available by Polly', () => {
    it('Language code by default should be en-US', () => {
        var settings = pollyConnection.setupParameters();
...
describe('Testing if sample rate only changes when it matches one of the pre-determined values', () => {
    it('Changing sample rate to 16000. Should be allowed', () => {
        var settings = pollyConnection.setupParameters();
        settings.sampleRate = '16000';
        expect(settings.sampleRate).toBe('16000');
    });
    it('Sending an integer as the parameter. Should not change value.', () => {
        var settings = pollyConnection.setupParameters();
        settings.sampleRate = 16000;
        expect(settings.sampleRate).toBe('24000');
    });
    it('Changing sample rate to a random number that is not one of the pre-determined values. Should not be changed', () => {
        var settings = pollyConnection.setupParameters();
        settings.sampleRate = '16000';
        settings.sampleRate = '8500';
        expect(settings.sampleRate).toBe('16000');
    });
    it('Sending an undefined value. Should not be changed', () => {
        var settings = pollyConnection.setupParameters();
